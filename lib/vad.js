...
export async function download(baseDir, onProgress, fetchImpl = globalThis.fetch) {
  ...
  for (const file of pending) {
    ...
    let response;
    try {
      // Con plazo. Una descarga que se queda a medias sin cortarse dejaba a la
      // página en «Bajando el detector» para siempre, sin error y sin salida.
      const plazo = typeof AbortSignal !== 'undefined' && typeof AbortSignal.timeout === 'function'
        ? { signal: AbortSignal.timeout(180_000) }
        : undefined;
      response = await fetchImpl(url, plazo);
    } catch (error) {
