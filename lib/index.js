    ...
    ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        collectBody(req, res, MAX_JSON_BODY, (body) => {
          ...
          // The voice is in use. If the window is wanted, this is the moment:
          // not at startup, when it would sit there for hours doing nothing.
          if (vset.overlayAuto && parsed && parsed.mode && parsed.mode !== 'idle') {
    ...
    ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        collectBody(req, res, MAX_JSON_BODY, (body) => {
          ...
          order = { seq: order.seq + 1, name, at: Date.now() };
          empujar(order);
  ...
  ctx.effect(() =>
    ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        const soltar = () => { abiertos.delete(res); };
        req.on('close', soltar);
        req.on('error', soltar);
        res.on('close', soltar);
        res.on('error', soltar);
