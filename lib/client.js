...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /* QUIÉN ES ESTA PÁGINA.
     *
     * El arnés puede estar abierto en dos sitios a la vez —la aplicación
     * instalada y una pestaña, o dos pestañas— y las dos recibían las mismas
     * órdenes: una tecla abría DOS micrófonos, dos transcripciones y dos
     * envíos, y el estado que veía la ventana flotante saltaba de una página
     * a la otra. Medido en uso. Cada página se presenta con un nombre propio;
     * el servidor decide a cuál le tocan las teclas —la que está usando la
     * voz— y las demás se quedan quietas. */
    const PAGE_ID = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;

    /** Cuánto texto se le pide a una voz de una vez. Más largo y la voz del
     *  sistema se calla a mitad sin avisar (Chrome), el servidor recorta sin
     *  decirlo, y un trozo que tarda en llegar es silencio que parece avería. */
    const TROZO_MAX = 220;
     ...
     */
    /* LAS VOCES DEL SISTEMA LLEGAN TARDE.
     *
     * Chrome devuelve una lista VACÍA hasta que las ha cargado, y avisa con
     * `voiceschanged`. Pedirlas justo al leer daba vacío, la primera frase
     * salía con la voz por defecto del navegador —a veces inglesa— y la
     * siguiente ya con la española: «la voz cambia sola». Se recogen al
     * arrancar y cada vez que el navegador avisa, y quien lee las pide ya
     * recogidas. */
    let vocesDelSistema = [];
    let sinVoces = false;   // ya se esperó una vez y no llegó ninguna: no se vuelve a esperar

    function recogerVoces() {
      try { vocesDelSistema = window.speechSynthesis ? window.speechSynthesis.getVoices() : []; }
      catch { vocesDelSistema = []; }
      return vocesDelSistema;
    }

    /** La lista, esperando un momento si todavía no ha llegado. */
    async function vocesListas() {
      if (vocesDelSistema.length || sinVoces || !window.speechSynthesis) return vocesDelSistema;
      for (let intento = 0; intento < 4; intento += 1) {
        if (recogerVoces().length) return vocesDelSistema;
        await new Promise((r) => setTimeout(r, 300));
      }
      sinVoces = !recogerVoces().length;
      return vocesDelSistema;
    }

    function elegirVozDelSistema(idioma, todas) {
      if (!Array.isArray(todas) || !todas.length) return null;
    ...
    /**
     * Read `texto` with the system voice. Resolves when it has finished — or
     * when it was stopped, or when it plainly is not going to finish.
     *
     * `voz` la elige quien lee, UNA vez por respuesta: elegirla aquí en cada
     * frase era una de las formas en que la voz cambiaba a mitad.
     */
    function leerConElSistema(texto, voz, { alSonar, ritmo } = {}) {
      return new Promise((resolve, reject) => {
        ...
        frase.rate = ritmo || ritmoDeLectura;
        ...
        frase.onstart = () => { if (alSonar) { try { alSonar(); } catch { /* un aviso no tumba la lectura */ } } };
        frase.onend = fin;
        frase.onerror = (e) => {
          ...
          if (parado) { fin(); return; }
          if (acabado) return;
          acabado = true;
          clearTimeout(vigia);
          clearInterval(latido);
          reject(new Error((e && e.error) || 'the built-in voice failed'));
        };

        /* EL VIGÍA. Chrome tiene una avería conocida: una frase se calla a
         * mitad y no dispara ni «end» ni «error». Sin esto la promesa no se
         * resolvía NUNCA: el ciclo se quedaba esperando una voz que ya había
         * callado, la barra en rojo, y ni la tecla de callar ni el botón de
         * la conversación sacaban de ahí. Desde fuera: «se queda atascado» y
         * «no puedo apagarlo». Se le da el tiempo que tardaría en leerlo y un
         * margen; pasado eso, se da por leída y se sigue. */
        const ritmoReal = Math.max(0.5, Number(frase.rate) || 1);
        vigia = setTimeout(() => {
          try { window.speechSynthesis.cancel(); } catch { /* ya callada */ }
          fin();
        }, Math.round(4000 + (frase.text.length * 110) / ritmoReal));

        /* Y EL LATIDO. La misma avería tiene el remedio de siempre: pausar y
         * reanudar cada pocos segundos mantiene viva la voz. Sólo con las
         * voces que no son locales, que son las que se caen. */
        if (voz && voz.localService === false) {
          latido = setInterval(() => {
            try { window.speechSynthesis.pause(); window.speechSynthesis.resume(); } catch { /* nada que hacer */ }
          }, 10000);
        }
    ...
    /* CADA «CALLAR» CUENTA.
     *
     * Un trozo que se estaba pidiendo al servidor cuando alguien mandó callar
     * llegaba medio segundo después y sonaba igual: callar no callaba. Quien
     * va a reproducir algo mira si el número ha cambiado desde que lo pidió,
     * y si ha cambiado, no suena. */
    let silencios = 0;
    ...
    /**
     * Partir lo que hay que decir en trozos que una voz aguante.
     *
     * Corta en finales de frase; una frase más larga que `max`, en comas; y
     * una sin comas, en espacios. Nunca a media palabra. Un punto seguido de
     * una cifra no es un final de frase: «54.5» se lee entero.
     *
     * Existe porque la respuesta entera se le pedía a la voz de una vez. Con
     * la del sistema, Chrome se callaba a mitad; con la neuronal, el servidor
     * recortaba a dos mil letras sin decirlo; y con Piper, una respuesta larga
     * se pasaba del tiempo que se le da y caía a la voz del sistema — que es
     * otra de las formas en que «la voz cambiaba sola».
     */
    function trocear(texto, max) {
      const limite = Number.isFinite(max) && max > 0 ? Math.floor(max) : 220;
      ...
      for (let i = 0; i < limpio.length; i += 1) {
        ...
        const cierra = c === '\n'
          || ('.!?…:'.includes(c) && (sig === undefined || /\s/.test(sig)));
        if (cierra) {
      ...
      const porEspacios = (frase) => {
        const piezas = [];
        let cur = '';
        for (const palabra of frase.split(/\s+/).filter(Boolean)) {
          const cand = cur ? `${cur} ${palabra}` : palabra;
          if (cand.length <= limite) { cur = cand; continue; }
          if (cur) piezas.push(cur);
          if (palabra.length > limite) {
            for (let i = 0; i < palabra.length; i += limite) piezas.push(palabra.slice(i, i + limite));
            cur = '';
          } else {
            cur = palabra;
          }
      ...
      const porComas = (frase) => {
        const piezas = [];
        let cur = '';
        for (const parte of frase.split(/(?<=[,;])\s*/)) {
          if (!parte) continue;
          if (parte.length > limite) {
            if (cur) { piezas.push(cur); cur = ''; }
            piezas.push(...porEspacios(parte));
            continue;
          }
          const cand = cur ? `${cur} ${parte}` : parte;
          if (cand.length <= limite) cur = cand;
          else { piezas.push(cur); cur = parte; }
        }
        if (cur) piezas.push(cur);
        return piezas;
      };
      ...
      for (const frase of frases) {
        if (frase.length > limite) {
          if (junto) { trozos.push(junto); junto = ''; }
          trozos.push(...porComas(frase));
          continue;
        }
        const cand = junto ? `${junto} ${frase}` : frase;
        if (cand.length <= limite) junto = cand;
        else { trozos.push(junto); junto = frase; }
      }
      if (junto) trozos.push(junto);
      return trozos;
    }

    /**
     * LA LECTURA: cómo se va a leer ESTA respuesta, decidido UNA vez.
     *
     * Motor, voz, ritmo y voz del sistema de reserva se fijan al empezar la
     * respuesta y no se tocan hasta la siguiente. Antes cada frase decidía por
     * su cuenta —el motor que había en memoria desde que se cargó la página,
     * la voz que tuviera el servidor en ese instante, la del sistema que
     * saliera— y una respuesta podía salir con tres voces. Se pregunta al
     * servidor aquí, así que un cambio hecho en la ventana flotante entra en
     * la respuesta siguiente sin recargar nada.
     */
    async function prepararLectura(reserva) {
      let cfg = null;
      try {
        const res = await fetch(`${location.origin}${BASE_PATH}/config`, { cache: 'no-store' });
        cfg = await res.json();
      } catch { /* se lee con lo último que se supo */ }
      if (cfg && cfg.ok) {
        fijarRitmo(cfg.speechRate);
        fijarIdiomaUI(cfg.uiLang);
        nombreDeAparato.input = cfg.micLabel || '';
        nombreDeAparato.output = cfg.outputLabel || '';
      }
      const motor = (cfg && cfg.speechEngine) || (reserva && reserva.motor) || 'none';
      const idioma = (cfg && cfg.language) || (reserva && reserva.idioma) || 'auto';
      const vozSistema = elegirVozDelSistema(idioma, await vocesListas());
      return {
        motor,
        voz: (cfg && cfg.resolvedVoice) || '',
        ritmo: ritmoDeLectura,
        idioma,
        vozSistema,
        // Se pone a verdadero cuando el motor no ha podido: el resto de la
        // respuesta va con la voz del sistema, y se dice.
        caido: false,
      };
    }

    /** Un trozo por el motor del servidor. Verdadero si sonó (o si se mandó
     *  callar mientras llegaba, que no es un fallo del motor). */
    async function decirConElMotor(trozo, lectura, alSonar) {
      const pedido = silencios;
      try {
        const res = await fetch(`${location.origin}${BASE_PATH}/speak`, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          // La voz y el ritmo van EXPLÍCITOS: los decidió esta lectura. Sin
          // esto, un cambio de voz en la ventana a mitad de respuesta entraba
          // en la frase siguiente.
          body: JSON.stringify({ text: trozo, voice: lectura.voz || undefined, rate: lectura.ritmo }),
        });
        const data = await res.json().catch(() => ({}));
        if (!(data && data.ok && (data.audioBase64 || data.wavBase64))) return false;
        if (pedido !== silencios) return true;
        await reproducir(data.audioBase64 || data.wavBase64, data.mime || 'audio/wav', alSonar);
        return true;
      } catch {
        return false;
      }
     ...
     *
     * `lectura` viene de prepararLectura(). `sigue()` se consulta entre trozo
     * y trozo: cuando dice que no, se para ahí. `alSonar()` se llama cuando
     * de verdad empieza a salir sonido, que no es cuando se pide.
     *
     * Returns how it was read: 'motor', 'sistema', 'caido' (the engine failed
     * and the rest went out with the built-in voice) or 'nada'.
     */
    async function hablarTexto(texto, lectura, { alSonar, sigue } = {}) {
      const dicho = paraLeer(texto);
      if (!dicho) return 'nada';
      const puedeSeguir = () => !sigue || sigue();

      let porMotor = false;
      for (const trozo of trocear(dicho, TROZO_MAX)) {
        if (!puedeSeguir()) break;

        const conMotor = (lectura.motor === 'piper' || lectura.motor === 'neural') && !lectura.caido;
        if (conMotor) {
          // Una segunda oportunidad: un tropiezo de red no es un motor caído.
          let sono = await decirConElMotor(trozo, lectura, alSonar);
          if (!sono && puedeSeguir()) sono = await decirConElMotor(trozo, lectura, alSonar);
          if (sono) { porMotor = true; continue; }
          /* SE CAE PARA TODA LA RESPUESTA. Antes el motor fallaba en UNA
           * frase, esa frase salía con la voz del sistema y la siguiente otra
           * vez con la neuronal: la voz cambiaba a mitad de respuesta. Si el
           * motor no puede, el resto de esta respuesta va con la del sistema.
           * La respuesta siguiente lo vuelve a intentar. */
          lectura.caido = true;
        }
        if (!puedeSeguir()) break;
        await leerConElSistema(trozo, lectura.vozSistema, { alSonar, ritmo: lectura.ritmo });
      }
      if (lectura.caido) return 'caido';
      return porMotor ? 'motor' : 'sistema';
    }

    /** Play one piece of synthesised audio through the chosen output. */
    async function reproducir(audioBase64, mime, alSonar) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const arrancar = () => {
          sonando = {
            audio,
            parar() { try { audio.pause(); audio.currentTime = 0; } catch { /* stopped */ } fin(); },
          };
          audio.onplaying = () => {
            if (alSonar) { try { alSonar(); } catch { /* un aviso no tumba la lectura */ } }
            /* EL VIGÍA: lo que dura el audio y un margen. Un audio que no
             * termina nunca —una salida que se desenchufa a mitad— dejaba la
             * lectura esperando para siempre. */
            const dura = Number.isFinite(audio.duration) && audio.duration > 0 ? audio.duration : 30;
            clearTimeout(vigia);
            vigia = setTimeout(() => fin(), Math.round(dura * 1000 + 5000));
          };
          audio.onended = () => fin();
          audio.onerror = () => fin(new Error('the audio could not be played'));
          // Hasta que empiece a sonar. Si no arranca en un rato, no va a arrancar.
          vigia = setTimeout(() => fin(new Error('the audio did not start')), 15000);
          audio.play().catch((error) => fin(error));
        };
     ...
     */
    async function leerSegunLlega(sesionAhora, { lectura, sigueVivo, alEmpezar, alSonar, anterior }) {
      ...
      while (sigueVivo()) {
        ...
        if (ahora.trim()) {
          ...
          await hablarTexto(ahora, lectura, { alSonar, sigue: sigueVivo });
          if (!sigueVivo()) break;
    ...
    function createBus() {
      ...
      const publish = (next) => {
        ...
        // Y quién lo dice: el servidor sólo se queda con el estado de la página
        // que está usando la voz, y le manda a ésa las teclas.
        const wire = JSON.stringify({ page: PAGE_ID, mode: next.mode, message: next.message, caption: next.caption, level,
      ...
      function atender(orden) {
        ...
        // Una orden con destinataria es para ELLA. Las demás páginas la ven
        // pasar y no hacen nada — que es justo lo que no hacían.
        if (orden.page && orden.page !== PAGE_ID) return;
      ...
      const abrirFlujo = () => {
        try {
          flujo = new EventSource(`${location.origin}${BASE_PATH}/orders?page=${encodeURIComponent(PAGE_ID)}`);
          flujo.onmessage = (e) => {
      ...
      /* CUANDO LA PERSONA VIENE A ESTA PÁGINA, las teclas son de esta página.
       * Con dos abiertas, la que manda es la que usa la voz; si ninguna la
       * está usando, la que acaba de recibir el foco. Se le dice al servidor,
       * que es quien reparte. */
      const alEnfocar = () => {
        try {
          void fetch(`${location.origin}${BASE_PATH}/state`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ page: PAGE_ID, focus: true }),
            keepalive: true,
          }).catch(() => { /* sin servidor no hay a quién decírselo */ });
        } catch { /* idem */ }
      };
      window.addEventListener('focus', alEnfocar);
      if (document.hasFocus && document.hasFocus()) alEnfocar();
      ...
      async function pollOrders() {
        ...
        try {
          const response = await fetch(
            `${location.origin}${BASE_PATH}/command?page=${encodeURIComponent(PAGE_ID)}`,
            { cache: 'no-store' });
    ...
    const L = {
      ...
      talkBusy: [
        'La conversación está en marcha: basta con hablar. Para dictar, termina antes la conversación.',
        'The conversation is on: just speak. To dictate, end the conversation first.',
        '对话进行中：直接说话即可。如需听写，请先结束对话。',
      ],
    ...
    function icono(cual) {
      const trazos = {
        ...
        // El modo KITT ya no lleva dibujo: es la letra K, ver TalkButton.
    ...
    function MicButton({ bus, useInput, inputActions, sessionId }) {
      ...
      const startRecording = useCallback(async () => {
        ...
        recorder.onstop = async () => {
          ...
          rafRef.current = null;
          // El mando ya no está en la página: lo grabado no tiene dónde caer.
          if (desmontado.current) { chunksRef.current = []; cleanupRec(); return; }
      ...
      /* LA CONVERSACIÓN VIVE FUERA DE ESTE COMPONENTE.
       *
       * Estaba en una referencia de React de este botón. El arnés quita y
       * vuelve a poner la fila de herramientas —al abrir los ajustes, al
       * cambiar de vista— y con ella se iba la referencia: el detector seguía
       * corriendo y mandando turnos, y el botón nuevo no sabía nada de él.
       * Desde fuera: una conversación que no se puede apagar. Ahora el mando
       * vive en el puente que comparten los botones y sobrevive a que se
       * repinten. */
      const pararCharla = useCallback((motivo) => {
        const c = micBridge.charla;
        micBridge.charla = null;
        // PRIMERO SE CALLA. Se cerraba el detector y se dejaba que la frase en
        // curso terminara: el botón de apagar no apagaba hasta que la voz
        // acababa el párrafo, y un párrafo largo son muchos segundos de «le he
        // dado y no hace caso».
        callar();
      ...
      const arrancarCharla = useCallback(async () => {
        /* UNA SOLA, Y LA GUARDIA ES SÍNCRONA.
         *
         * La comprobación de «ya hay una» se hacía después de dos esperas
         * —comprobar y cargar el detector— y dos pulsaciones seguidas pasaban
         * las dos: dos detectores, dos micrófonos, dos turnos por cada frase.
         * «Nacen dos.» La bandera se pone antes de la primera espera. */
        if (micBridge.charla || micBridge.arrancando) return;
        micBridge.arrancando = true;
        ...
        const abrir = async () => {
          ...
          let cerrado = false;
          let leyendo = false;      // está SONANDO la respuesta: el oído no entrega turnos
           ...
           *
           * Estas tres cosas son todo lo que hace falta: cuando empezó a
           * SONAR, cuánto eco entra por el micrófono en esta máquina, y cuánto
           * rato lleva sonando algo que supera ese eco.
           *
           ...
           *
           * NO SE ADIVINA UN UMBRAL, SE MIDE. Durante el primer medio segundo
           * de SONIDO de cada lectura lo que entra por el microfono ES el eco,
           * por definicion, porque todavia no ha hablado nadie. Ese es el suelo
           * de esta maquina en esta habitacion. Para contar como voz hay que
           * pasarlo con holgura y sostenerlo.
           *
           * Y EMPIEZA A MEDIR CUANDO SUENA, no cuando se pide. Empezaba al
           * pedir la primera frase, y con la voz neuronal entre pedirla y oírla
           * pasa un segundo de silencio: se calibraba contra la habitación
           * callada, el suelo salía casi cero, y en cuanto la respuesta
           * empezaba a sonar por el altavoz su propio eco pasaba el listón y
           * la cortaba. «Se para sola, no sabemos por qué.» Medido. */
        ...
        function mirarCorte(estado, energia, ahora) {
          ...
          /* EL OÍDO SE TOCA DE UNO EN UNO, Y ESPERANDO.
           *
           * `pause()` y `start()` del detector son asíncronos, y no son
           * simétricos: pause marca «no escucho», suelta el micrófono y
           * DESPUÉS para el procesador; start marca «escucho», reanuda el
           * procesador y DESPUÉS vuelve a pedir el micrófono. Llamados seguidos
           * sin esperar —que es lo que hacía el corte de voz—, el «parar» de
           * pause llegaba después del «reanudar» de start: micrófono abierto,
           * procesador parado. Sordo, con la barra en azul. Y como start ya
           * había marcado «escucho», ningún start posterior hacía nada. Era el
           * «se corta el flujo al segundo mensaje». Medido contra el código del
           * propio detector, no supuesto.
           *
           * Así que cada paso espera al anterior, y un start que falla se
           * cuenta, porque el detector no lo cuenta: se queda marcado como
           * escuchando sin micrófono. */
          let cola = Promise.resolve();
          const alOido = (paso) => {
            const p = cola.then(async () => {
              if (!vad || cerrado) return;
              await paso(vad);
            });
            cola = p.catch(() => { /* el siguiente paso decide por su cuenta */ });
            return p;
          };
          const cerrarOido = () => alOido((v) => v.pause());
          const abrirOido = () => alOido((v) => v.start());
          ...
          const charla = {
            cerrar() {
              ...
              const v = vad;
              vad = null;
              // Detrás de lo que esté en marcha: destruir a mitad de un
              // arranque deja un micrófono abierto sin dueño.
              if (v) cola = cola.then(() => v.destroy()).catch(() => { /* going anyway */ });
            },
            ...
            silenciar() { void cerrarOido().catch(() => { /* ya cerrado */ }); },
            escuchar() {
              void abrirOido().catch((error) => {
                bus.set({ mode: 'error', message: `${t(L.micLost)} (${error?.message ?? error})` });
              });
            },
            /* CALLAR, dentro de una conversación: se corta la lectura en curso
             * Y se deja de esperar. La tecla de callar sólo cortaba la frase en
             * curso y el ciclo seguía con la siguiente medio segundo después:
             * callarse una frase y seguir con la otra es no callarse. */
            cortar() { cortado = true; callar(); },
          };
          micBridge.charla = charla;
          ...
          const turno = async (audio) => {
            ...
            bus.set({ mode: 'transcribing', level: 0, caption: null, reading: false });
            ...
            try {
              ...
              const data = await res.json().catch(() => ({}));
              if (!data.ok) { if (vigente()) avisarSinColgar(data.reason || t(L.sttFailed)); return; }
              texto = String(data.text || '').trim();
            } catch (error) {
              if (vigente()) avisarSinColgar(`${t(L.sttFailed)}: ${error?.message ?? error}`);
              return;
            }
            ...
            if (!vigente()) return;
            // Nada reconocible: al reposo, no a «Escuchando». Ponía «Escuchando»
            // y se quedaba así hasta la siguiente voz, que es justo la palabra
            // que no quieres leer clavada en la pantalla.
            if (!texto) { bus.set({ mode: 'armed', caption: null }); return; }
            ...
            try {
              const res = await fetch(
                `${location.origin}${BASE_PATH}/last?live=1&sessionId=${encodeURIComponent(sesionRef.current || '')}`,
                { cache: 'no-store' });
              const data = await res.json().catch(() => ({}));
            ...
            // Cómo se va a leer ESTA respuesta, decidido ahora y para toda ella.
            const lectura = await prepararLectura({ motor: engineRef.current, idioma: langRef.current });
            if (!vigente()) return;
            ...
            // was spoken to, and the thing talks to itself forever. So while the
            // reply SOUNDS, anything the detector delivers is thrown away, and
            // only a voice that clears the measured echo floor counts.
            //
            // PERO NO SE CIERRA AL ENVIAR, SINO AL EMPEZAR A SONAR. Mientras el
            // agente piensa el oído está abierto: no hay eco del que defenderse.
            try {
              await leerSegunLlega(() => sesionRef.current, {
                lectura,
                anterior,
                // Se deja de leer en cuanto entra un turno nuevo o alguien pide
                // la palabra: no basta con oir, hay que callarse al oir.
                sigueVivo: () => vigente() && !cortado,
                alEmpezar: () => {
                  bus.set({ mode: 'reading', message: null, caption: null, reading: true, coverMicUntil: Date.now() + 120000 });
                },
                alSonar: () => {
                  // AHORA SÍ SUENA. El oído se cierra a los turnos y empieza a
                  // medir el eco: el primer medio segundo de sonido es el suelo.
                  if (!vigente() || cortado) return;
                  leyendo = true;
                  if (!corte) corte = nuevoCorte(Date.now());
                },
              });
            } catch (error) {
              if (vigente()) bus.set({ mode: 'error', message: String(error?.message ?? error) });
            }

            /* EL FINAL DE LA LECTURA, CON CUIDADO.
             *
             * El detector lleva dentro el eco de la última frase: una voz que
             * empezó mientras sonaba la respuesta y que todavía no ha terminado
             * para él, porque espera su segundo y medio de silencio. Antes se
             * abría el oído a los 500 ms y ese final llegaba después, ya con el
             * oído abierto: se transcribía la propia respuesta del plugin y se
             * le mandaba al agente como si lo hubiera dicho la persona. «Se
             * mezcla», «habla solo». Pausar el detector vacía lo que llevaba
             * dentro; arrancarlo, no. Se pausa, se espera, y se arranca.
             *
             * Si fue un corte de voz, el corte ya vació el detector y ya puso
             * «Escuchando»: aquí no se toca nada, que la persona está hablando. */
            const fueCortado = cortado;
            if (!fueCortado) {
              await cerrarOido().catch(() => { /* sigue como pueda */ });
              // The air takes a moment to go quiet after the speaker stops.
              await new Promise((r) => setTimeout(r, REOPEN_MIC_MS));
            }
            // Si ya manda otro turno, el estado es suyo: este no lo toca.
            if (!vigente()) return;
            leyendo = false;
            ecoEnCurso = false;
            corte = null;
            ...
            if (cerrado) return;
            if (fueCortado) return;

            // Silenciado a mano: el oído se queda cerrado, que es lo que pidió.
            if (bus.getState().muted) {
              bus.set({ mode: 'armed', caption: null, message: lectura.caido ? t(L.systemVoiceInstead) : null });
              return;
            }
            // Si el oído no vuelve a abrirse, la conversación está encendida y
            // sorda: el peor sitio para callarse. Se dice.
            try { await abrirOido(); }
            catch (error) {
              if (vigente()) bus.set({ mode: 'error', message: `${t(L.micLost)} (${error?.message ?? error})` });
              return;
            }
          ...
          try {
            ...
            /* EL MICRÓFONO DEL DETECTOR ES EL QUE SE ELIGIÓ.
             *
             * El detector abre SU PROPIO micrófono, y lo abría con el de por
             * defecto del sistema: el que se elige en el menú sólo valía para
             * pulsar y hablar. Y cada vez que se para y se arranca —al acabar
             * una respuesta, al silenciar, al cortar— lo vuelve a pedir, así
             * que se le dan las dos puertas, la de arrancar y la de reanudar,
             * y las dos piden el mismo aparato con los mismos filtros. Eco
             * cancelado SIEMPRE: es la diferencia entre poder hablar con los
             * altavoces puestos y no poder. */
            const abrirMicrofono = async () => {
              const elegido = await idDeAparato('input');
              return navigator.mediaDevices.getUserMedia({
                audio: {
                  ...
                  ...(elegido ? { deviceId: { exact: elegido } } : {}),
                },
              });
            };
            vad = await lib.MicVAD.new({
              ...VAD_SETTINGS,
              getStream: abrirMicrofono,
              resumeStream: abrirMicrofono,
              ...
              onFrameProcessed: (probs, frame) => {
                ...
                /* MIENTRAS SUENA: se mide el eco y se decide si alguien esta
                ...
                if (leyendo) {
                  ...
                  if (mirarCorte(corte, energia, Date.now()) === 'cortar') {
                    ...
                    // se arranca —DE UNO EN UNO, ver arriba—, que vacía lo que
                    // llevaba dentro. Cuesta el arranque de la primera palabra, y
                    // es un precio mucho menor que mandar a transcribir la propia
                    // respuesta.
                    void cerrarOido().catch(() => { /* sigue como pueda */ });
                    void abrirOido().catch((error) => {
                      bus.set({ mode: 'error', message: `${t(L.micLost)} (${error?.message ?? error})` });
                    });
                ...
                // quien sabe en qué unidades mide. Y va SOBRADA de ganancia a
                // propósito: esto es un indicador para mirar de reojo, no un
                // instrumento de medida.
                bus.set({ level: Math.min(1, energia * 14) });
              },
              onVADMisfire: () => {
                ecoEnCurso = false;
                // Ha sido un ruido, no una voz: vuelve al reposo sin más. Pero
                // SÓLO desde «Escuchando»: un ruido mientras piensa o transcribe
                // borraba esas palabras de la pantalla y parecía que se había
                // parado.
                const modo = bus.getState().mode;
                bus.set({ level: 0, mode: modo === 'listening' ? 'armed' : modo });
              },
              onSpeechEnd: (audio) => {
          ...
          // Esperando: el primer arranque es el que pide el micrófono, y si no
          // lo dan, aquí es donde hay que decirlo — no un segundo después con
          // la conversación ya «encendida».
          try { await abrirOido(); }
        ...
        try {
          ...
          micBridge.arrancando = false;
        }
      }, [bus, inputActions, pararCharla, avisarSinColgar]);
      ...
      const onClick = () => {
        ...
        if (s.muted) return;
        /* CON LA CONVERSACIÓN EN MARCHA, ESTE MANDO NO ABRE OTRO MICRÓFONO.
         * Lo abría: el detector escuchando y una grabación encima, dos
         * transcripciones de lo mismo y dos textos en la caja. Se dice y no
         * se hace nada. */
        if (micBridge.charla) { bus.set({ message: t(L.talkBusy) }); return; }
        if (s.mode === 'listening') stopRecording();
        else void startRecording();
      };

      /* SI LA PÁGINA QUITA ESTE MANDO A MITAD DE UN DICTADO, el micrófono no
       * se queda abierto y solo. La conversación no se toca: vive fuera de
       * aquí a propósito, y el botón que vuelva la encontrará. */
      ...
      useEffect(() => {
        ...
        return () => {
          ...
          speechRef.current = null;
          if (r) { try { r.abort(); } catch { /* ya parado */ } }
          cleanupRec();
        };
      }, [cleanupRec]);
      ...
      // Dentro de una conversación, callar corta la lectura Y la espera.
      useEffect(() => bus.onOrder('stop-speaking', () => {
        if (micBridge.charla) { try { micBridge.charla.cortar(); } catch { /* ya callada */ } }
      }));
    ...
    function SpeakButton({ bus, sessionId }) {
      ...
      const startReading = useCallback(async () => {
        ...
        try {
          ...
          // Cómo se lee ÉSTA, decidido una vez: motor, voz y ritmo de ahora
          // mismo, para toda la respuesta.
          const lectura = await prepararLectura({ motor: engineRef.current, idioma: langRef.current });
          if (!bus.getState().reading) return;   // lo pararon mientras se preparaba

          const como = await hablarTexto(lastData.text, lectura, { sigue: () => bus.getState().reading });
          ...
          if (bus.getState().reading) {
            bus.set({
              mode: 'idle',
              // Say which voice it was ONLY when it was not the one asked for.
              message: como === 'caido' ? t(L.systemVoiceInstead) : null,
              reading: false,
              coverMicUntil: Date.now() + REOPEN_MIC_MS,
            });
          }
    ...
    function StatusBar({ bus }) {
      ...
      // Un aviso que no es avería —«leído con la voz del sistema», «la
      // conversación está en marcha»— también se enseña. Se guardaba en el
      // estado y no lo pintaba nadie.
    ...
    function TalkButton({ bus }) {
      ...
      /* LA K, como en la barra flotante. El modo KITT no lleva bocadillo: es la
       * letra de la marca, azul mientras está en marcha. Así el modo especial
       * se distingue de los mandos normales de un vistazo, en los dos sitios. */
      return React.createElement('button', {
        type: 'button',
        title,
        'aria-label': title,
        'aria-pressed': enCharla ? 'true' : 'false',
        onClick,
        style: toolButtonStyle('talk', enCharla),
      }, React.createElement('b', {
        style: { fontSize: 12, fontWeight: 800, lineHeight: 1, letterSpacing: 0, fontFamily: 'Segoe UI, Inter, system-ui, sans-serif' },
        'aria-hidden': 'true',
      }, 'K'));
    }
    ...
    function apply(ctx) {
      ...
      // Las voces del sistema, recogidas desde el principio y cada vez que el
      // navegador avise: quien lee no puede esperar a que lleguen.
      if (window.speechSynthesis) {
        recogerVoces();
        try { window.speechSynthesis.addEventListener('voiceschanged', recogerVoces); } catch { /* sin evento */ }
        if (typeof ctx.effect === 'function') {
          ctx.effect(() => () => {
            try { window.speechSynthesis.removeEventListener('voiceschanged', recogerVoces); } catch { /* nada */ }
          });
        }
      }

      // El mismo orden que la barra flotante, de izquierda a derecha: la K del
      // modo KITT, la web, y los mandos normales — hablar, leer, silenciar,
      // opciones.
      ctx.slots.inject(
        'conversation.input.right',
        () => ctx.slots.register(
          { name: 'conversation.input.right', id: 'dsh-kitt-voice-talk', order: 78, inject: () => ({ bus }) },
          TalkButton
        )
      );
      ...
      ctx.slots.inject(
        'conversation.input.right',
        () => ctx.slots.register(
          { name: 'conversation.input.right', id: 'dsh-kitt-voice-speak', order: 81, inject: () => ({ bus }) },
          SpeakButton
        )
      );
      ...
      ctx.slots.inject(
        'conversation.input.right',
        () => ctx.slots.register(
          { name: 'conversation.input.right', id: 'dsh-kitt-voice-mute', order: 82, inject: () => ({ bus }) },
          MuteButton
        )
      );
