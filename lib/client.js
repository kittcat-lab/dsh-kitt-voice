    ...
    function MicButton({ bus, useInput, inputActions, sessionId }) {
      ...
      const startRecording = useCallback(async () => {
        ...
        try {
          ...
          stream = await navigator.mediaDevices.getUserMedia({
            audio: chosenInput ? { deviceId: { exact: chosenInput } } : true,
          });
        } catch {
      ...
      const arrancarCharla = useCallback(async () => {
        ...
        const turno = async (audio) => {
          ...
          //
          // It is paused HERE, before the reply is read rather than after it
          // is complete, because the reading now starts while the agent is
          // still writing.
          leyendo = true;
          if (vad) { try { vad.pause(); } catch { /* the flags still guard it */ } }
          bus.set({ reading: true, coverMicUntil: Date.now() + 120000 });
        ...
        try {
          ...
          vad = await lib.MicVAD.new({
            ...
            onFrameProcessed: (probs, frame) => {
              if (leyendo || ecoEnCurso || cerrado) return;
